  ...
  useEffect(() => {
    ...
    fetch(`/api/shopify/callback?${queryParams.toString()}`)
      ...
      .then((data) => {
        ...
        if (data.status && data.storefrontAccessToken?.accessToken) {
          const tokenValue = data.storefrontAccessToken.accessToken
          setToken(tokenValue)
          sessionStorage.setItem("shopify_storefront_token", tokenValue)
        } else {
          setTokenError(true)
        }
